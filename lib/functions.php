<?php
	...
	function carrega_pagina($con, $data, $isAdmin){
		if($isAdmin == true){ //Se $isAdmin for Verdade ele vai para painel de adm.
			$url = (isset($_GET['pagina'])) ? $_GET['pagina'] : 'login';
			$explode = explode('/', $url);
			protegePagina($explode['0'], true);
		} else { //se não, vai para principal.
		...
		$dir = "pags/";
		$ext = ".php";
		// protegePagina($explode['0'], true);// Movido para cá para garantir que $explode esteja definida
		//Atualiza: Consegui botar dentro da verificação.
	...
	function protegePagina($explode, $isAdmin){
		$paginas_protegidas = array("inicio", "publicar", "gerenciar-posts", "editar-post", "deletar-post", "perfil");
				// se não estiver logado e tentar acessar as páginas protegidas...
